...
void OpenGLMeshPrimitive::render(SceneRenderer* renderer)
{
	...
	if(!renderer->isPicking() && _hasAlpha && !_triangleCoordinates.empty()) {
		...
		(glDrawElements(GL_TRIANGLES, _vertexBuffer.elementCount() * _vertexBuffer.verticesPerElement(), GL_UNSIGNED_INT, nullptr));
		primitiveIndices.oglBuffer().release();
	}
	else {
		// Render faces in arbitrary order.
		(glDrawArrays(GL_TRIANGLES, 0, _vertexBuffer.elementCount() * _vertexBuffer.verticesPerElement()));
	}
